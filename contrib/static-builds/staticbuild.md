...
make clean
export PATH=$PATH_orig
#transfer ~/sign/hemp0x-*-win*-unsigned.tar.gz to the signing machine
```
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/hemp0x-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer hemp0x-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
