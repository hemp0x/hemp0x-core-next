...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFlushOnClose)
{
    ...
    // Notify a trusted external command when a wallet transaction comes in or is updated.
    ...
    for (unsigned int i = 0; i < tx->vout.size(); ++i)
    {
        ...
        if (AreAssetsDeployed()) {
            if (txout.scriptPubKey.IsAssetScript()) {
                ...
                assetoutput.vout = i;
                if (GetAssetData(txout.scriptPubKey, assetoutput)) {
...
void CWallet::AvailableCoinsAll(std::vector<COutput>& vCoins, std::map<std::string, std::vector<COutput> >& mapAssetCoins, bool fGetHEMP, bool fGetAssets, bool fOnlySafe, const CCoinControl *coinControl, const CAmount& nMinimumAmount, const CAmount& nMaximumAmount, const CAmount& nMinimumSumAmount, const uint64_t& nMaximumCount, const int& nMinDepth, const int& nMaxDepth) const {
    vCoins.clear();

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if (fGetHEMP) { // Looking for HEMP Tx OutPoints Only
                    if (fHEMPLimitHit) // We hit our limit
                        continue;

                    // We only want HEMP OutPoints. Don't include Asset OutPoints
                    if (isAssetScript)
                        continue;
                                   ...
                                   const AssetType& assetType, bool sign)
{
    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-hemp0x-address
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    /** HEMP START */
                    // Check to see if you need to make an asset data outpoint OP_HEMP_ASSET data
                    ...
                    if (IsDust(txout, ::dustRelayFee) && !IsScriptTransferAsset(recipient.scriptPubKey)) /** HEMP START */ /** HEMP END */
                    {
