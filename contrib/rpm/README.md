...
The RPM spec file provided here is for Hemp0x-Core 0.12.0 and builds on CentOS
...
[Hemp0x git repository](https://github.com/hemp0x/hemp0x-core) but are not part of
...
necessary, the packager should bring the problem to the attention of the hemp0x
...
needed when 0.12.1 is released, a proper fix is already in the Hemp0x git
master branch.
...
idea to build Hemp0x Core against a static version of that library compiled
...
matters for the `hemp0x-server` package. At this point, most RPM based
...
The `hemp0x-server` package should have SELinux support. How to properly do
...
Hemp0x-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/hemp0x/). He has been
packaging Hemp0x for Fedora at least since 2012.
