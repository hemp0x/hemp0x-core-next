...
find ${DISTNAME}/bin -type f -executable -exec ../hemp0x/contrib/devtools/split-debug.sh {} {} {}.dbg \;
find ${DISTNAME}/lib -type f -exec ../hemp0x/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo hemp0x-*.tar.gz`
mkdir -p ~/hemp0x/temp
cd ~/hemp0x/temp
tar xf ../$SOURCEDIST
find hemp0x-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/hemp0x
...
cp -f hemp0x-*setup*.exe ~/release/unsigned/
mkdir -p ~/win64
...
cd ~/
rm -rf win64
cp -rf hemp0x/contrib/windeploy ~/sign/win64
cd ~/sign/win64/windeploy
mkdir -p unsigned
mv ~/hemp0x/hemp0x-*setup-unsigned.exe unsigned/
...
cd ~/
rm -rf win32
cp -rf hemp0x/contrib/windeploy ~/sign/win32
