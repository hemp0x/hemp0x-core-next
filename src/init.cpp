...
// Copyright (c) 2017-2021 The Hemp0x Core developers
...
static bool DefaultConfigFileExists()
{
    return fs::exists(GetConfigFile(HEMP0X_CONF_FILENAME)) || fs::exists(GetConfigFile(HEMP0X_ALTERNATE_CONF_FILENAME));
}

static std::string RandomHexString(size_t bytes)
{
    std::vector<unsigned char> data(bytes);
    GetStrongRandBytes(data.data(), data.size());
    return HexStr(data);
}

static bool CreateDefaultConfigFile(fs::path pathConfigFile, std::string& error)
{
    const fs::path configDir = pathConfigFile.parent_path();
    if (!fs::exists(configDir) && !TryCreateDirectories(configDir)) {
        error = strprintf("could not create configuration directory %s", pathConfigFile.parent_path().string());
        return false;
    }

    FILE* file = fsbridge::fopen(pathConfigFile, "w");
    if (file == nullptr) {
        error = strprintf("could not create %s", pathConfigFile.string());
        return false;
    }
#ifndef WIN32
    chmod(pathConfigFile.string().c_str(), S_IRUSR | S_IWUSR);
#endif

    const std::string rpcUser = "hemp0xrpc_" + RandomHexString(4);
    const std::string rpcPassword = RandomHexString(32);
    const std::string contents =
        "# Hemp0x Core configuration file\n"
        "#\n"
        "# This file was created automatically on first launch. Options are\n"
        "# disabled by default; remove the leading # to enable an option.\n"
        "# Command-line options override values set here. Restart hemp0xd after\n"
        "# editing this file.\n"
        "#\n"
        "# Local RPC is enabled for hemp0x-cli and local wallet applications.\n"
        "# Keep RPC bound to loopback unless you fully understand the exposure.\n"
        "#\n"
        "server=1\n"
        "rpcbind=127.0.0.1\n"
        "rpcallowip=127.0.0.1\n"
        "#\n"
        "# Cookie authentication works by default for local RPC clients.\n"
        "# Uncomment the generated credentials below only if a service needs\n"
        "# static RPC credentials, then restart hemp0xd.\n"
        "#rpcuser=" + rpcUser + "\n"
        "#rpcpassword=" + rpcPassword + "\n"
        "#rpcport=8766\n"
        "#\n"
        "# Node operation. The default mainnet P2P port is 42069.\n"
        "# daemon=1 is for Unix-style daemon mode; on Windows, use a service\n"
        "# manager or launch hemp0xd.exe directly.\n"
        "#\n"
        "#daemon=1\n"
        "#listen=1\n"
        "#port=42069\n"
        "#\n"
        "# Optional indexes. Explorers, public APIs, and some wallet services may\n"
        "# need these. Ordinary wallet nodes usually do not.\n"
        "#\n"
        "#txindex=1\n"
        "#addressindex=1\n"
        "#assetindex=1\n"
        ...
        "#dbcache=4000\n"
        "#\n"
        "# Optional ZMQ notifications for indexers and services.\n"
        "#zmqpubrawtx=tcp://127.0.0.1:28332\n"
        "#zmqpubrawblock=tcp://127.0.0.1:28333\n"
        "#\n"
        "# Optional block-template payout address for services that request block\n"
        "# templates from this node. Replace with your own Hemp0x address.\n"
        "#miningaddress=replace-with-your-hemp0x-address\n"
        "#\n"
        "# Optional seed peers. Uncomment these if the node has trouble finding\n"
        "# peers through the normal discovery path.\n"
    ...
    if (!ok) {
        error = strprintf("could not write %s", pathConfigFile.string());
        return false;
    }
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-alertnotify=<cmd>", _("Execute shell command when a relevant alert is received or we see a really long fork (%s in cmd is replaced by message; only use trusted commands)"));
    strUsage += HelpMessageOpt("-blocknotify=<cmd>", _("Execute shell command when the best block changes (%s in cmd is replaced by block hash; only use trusted commands)"));
    if (showDebug)
        strUsage += HelpMessageOpt("-blocksonly", strprintf(_("Whether to operate in a blocks only mode (default: %u)"), DEFAULT_BLOCKSONLY));
    strUsage +=HelpMessageOpt("-assumevalid=<hex>", strprintf(_("If this block is in the chain assume that it and its ancestors are valid and potentially skip their script verification (0 to verify all, default: %s, testnet: %s)"), defaultChainParams->GetConsensus().defaultAssumeValid.GetHex(), testnetChainParams->GetConsensus().defaultAssumeValid.GetHex()));
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s, fallback: hemp0x.conf)"), HEMP0X_CONF_FILENAME));
    if (mode == HMM_HEMP0XD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), HEMP0X_PID_FILENAME));
#endif
    ...
    if (showDebug) {
        strUsage += HelpMessageOpt("-acceptnonstdtxn", strprintf("Relay and consider \"non-standard\" transactions for block templates (%sdefault: %u)", "testnet/regtest only; ", !testnetChainParams->RequireStandard()));
        strUsage += HelpMessageOpt("-incrementalrelayfee=<amt>", strprintf("Fee rate (in %s/kB) used to define cost of relay, used for mempool limiting and BIP 125 replacement. (default: %s)", CURRENCY_UNIT, FormatMoney(DEFAULT_INCREMENTAL_RELAY_FEE)));
        strUsage += HelpMessageOpt("-dustrelayfee=<amt>", strprintf("Fee rate (in %s/kB) used to defined dust, the value of an output such that it will cost more than its value in fees at this fee rate to spend it. (default: %s)", CURRENCY_UNIT, FormatMoney(DUST_RELAY_TX_FEE)));
    }
    strUsage += HelpMessageOpt("-bytespersigop", strprintf(_("Equivalent bytes per sigop in transactions for relay and block-template policy (default: %u)"), DEFAULT_BYTES_PER_SIGOP));
    strUsage += HelpMessageOpt("-datacarrier", strprintf(_("Relay and consider data carrier transactions for block templates (default: %u)"), DEFAULT_ACCEPT_DATACARRIER));
    strUsage += HelpMessageOpt("-datacarriersize", strprintf(_("Maximum size of data in data carrier transactions we relay or include in block templates (default: %u)"), MAX_OP_RETURN_RELAY));
    strUsage += HelpMessageOpt("-mempoolreplacement", strprintf(_("Enable transaction replacement in the memory pool (default: %u)"), DEFAULT_ENABLE_REPLACEMENT));
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in %s/kB) smaller than this are considered zero fee for relaying, block-template policy, and transaction creation (default: %s)"),
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/Hemp0xProject/Hemp0x>";
    const std::string URL_WEBSITE = "<https://hemp0x.com>";
...
/** Sanity checks
 *  Ensure that Hemp0x is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Hemp0x process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    const bool customConfig = gArgs.IsArgSet("-conf");
    fs::path pathConfigFile = GetConfigFile(gArgs.GetArg("-conf", HEMP0X_CONF_FILENAME));
    if (!customConfig && !fs::exists(pathConfigFile) && fs::exists(GetConfigFile(HEMP0X_ALTERNATE_CONF_FILENAME))) {
        pathConfigFile = GetConfigFile(HEMP0X_ALTERNATE_CONF_FILENAME);
    }

    if (!customConfig && !DefaultConfigFileExists()) {
        std::string configError;
        if (CreateDefaultConfigFile(pathConfigFile, configError)) {
            LogPrintf("Created default config file %s\n", pathConfigFile.string());
        } else {
            LogPrintf("Config file not found: %s (running with defaults; %s)\n", pathConfigFile.string(), configError);
        }
    } else if (!fs::exists(pathConfigFile)) {
