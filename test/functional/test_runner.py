...
`test/functional/test_framework/test_framework.py:Hemp0xTestFramework.main`.
...
BASE_SCRIPTS= [
    ...
    'rpc_asset_purgesnapshot.py',
    'rpc_generation_disabled.py',
    'feature_proxy.py',
    'rpc_txindex.py',
...
def main():
    ...
    tmpdir = "%s/hemp0x_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and hemp0xd
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_hemp0xd = config["components"].getboolean("ENABLE_HEMP0XD")
    if not (enable_wallet and enable_cli and enable_hemp0xd):
        print("No functional tests to run. Wallet, utils, and hemp0xd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if hemp0xd is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "hemp0xd"]) is not None:
            print("%sWARNING!%s There is already a hemp0xd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "HEMP0XD" not in os.environ:
        os.environ["HEMP0XD"] = build_dir + '/src/hemp0xd' + exeext
        os.environ["HEMP0XCLI"] = build_dir + '/src/hemp0x-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `hemp0x-cli help` (`rpc_interface.txt`).
