...
The RPC interface might change from one major version of Hemp0x Core to the
...
The RPC interface allows other programs to control Hemp0x Core,
including the ability to spend funds from your wallets, affect consensus
verification, read private data, and otherwise perform operations that
can cause loss of money, data, or privacy.  This section suggests how
you should use and configure Hemp0x Core to reduce the risk that its
RPC interface will be abused.

- **Securing the executable:** Anyone with physical or remote access to
  the computer, container, or virtual machine running Hemp0x Core can
  compromise either the whole program or just the RPC interface.  This
  includes being able to record any passphrases you enter for unlocking
  your encrypted wallets or changing settings so that your Hemp0x Core
  program tells you that certain transactions have multiple
  confirmations even when they aren't part of the best block chain.  For
  this reason, you should not use Hemp0x Core for security sensitive
...
- **Securing local network access:** By default, the RPC interface can
  ...
  reason, it is important to only use Hemp0x Core for
  security-sensitive operations on a computer whose other programs you
  trust.

- **Securing remote network access:** You may optionally allow other
  computers to remotely control Hemp0x Core by setting the `rpcallowip`
  ...
  Internet.**  Although Hemp0x Core's RPC interface does use
  ...
  `hemp0xd -help` for more information about these settings and other
  settings described in this document.

    Related, if you use Hemp0x Core inside a Docker container, you may
...
- **Secure authentication:** By default, Hemp0x Core generates unique
  ...
  readable only by the user that started Hemp0x Core, allowing any of
