...
"""
Hemp0x P2P network half-a-node.

This python code was modified from ArtForz' public domain  half-a-node, as
found in the mini-node branch of http://github.com/jgarzik/pynode.

NodeConn: an object which manages p2p connectivity to a hemp0x node
...
class NodeConnCB:
    """Callback and helper functions for P2P connection to a hemp0xd node.
...
class NodeConn(asyncore.dispatcher):
    ...
    MAGIC_BYTES = {
        "mainnet": b"\x48\x45\x4d\x50",  # HEMP
        "testnet3": b"\x48\x54\x4e\x54",  # HTNT
        "regtest": b"\x48\x52\x45\x47",  # HREG
    }
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=NODE_NETWORK, send_version=True):
        ...
        logger.info('Connecting to Hemp0x Node: %s:%d' % (self.dstaddr, self.dstport))
